...
import com.google.light.server.dto.thirdparty.google.gdata.gdoc.GoogleDocImportBatchJobContext.State;
...
public class JobManagerImpl implements JobManager {
  ...
  public JobEntity enqueueGoogleDocImportBatchJob(GoogleDocImportBatchJobContext jobRequest) {
    Objectify ofy = ObjectifyUtils.initiateTransaction();
    try {
      jobRequest.setState(State.ENQUEUED);
  ...
  private void publishModule(GoogleDocImportJobContext gdocImportContext, JobEntity jobEntity) {
    try {
      ...
      Objectify ofy = ObjectifyUtils.initiateTransaction();
      try {
        gdocImportContext.setState(GoogleDocImportJobContext.State.COMPLETE);
  ...
  private void downloadArchiveIfReady(GoogleDocImportJobContext gdocImportContext,
    ...
    gdocImportContext.setState(GoogleDocImportJobContext.State.ARCHIVE_DOWNLOADED);
  ...
  private void startArchivingGoogleDoc(GoogleDocImportJobContext gdocImportContext,
    ...
    gdocImportContext.setState(GoogleDocImportJobContext.State.WAITING_FOR_ARCHIVE);
      ...
      if (version != null) {
        ...
        try {
          ...
          GoogleDocImportJobContext childJobContext = new GoogleDocImportJobContext.Builder()
              ...
              .moduleId(moduleId)
              .version(version)
              .state(GoogleDocImportJobContext.State.ENQUEUED)
              .build();
